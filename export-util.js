...
window.NCExport = (function () {
  ...
  // headers: string[]; rows: array of arrays (aynı sırada, header sayısı kadar hücre)
  function download(format, headers, rows, filenameBase) {
    const ts = new Date().toISOString().slice(0, 10);
    const filename = `${filenameBase}_${ts}`;
    const safeRows = rows.map(r => headers.map((h, i) => r[i] ?? ''));

    if (format === 'csv') {
      const csvRows = [headers, ...safeRows].map(r =>
    ...
    if (format === 'pdf') {
      ...
      doc.setFontSize(11);
      doc.text(filenameBase.replace(/_/g, ' '), 24, 24);
      doc.autoTable({
        head: [headers],
        body: safeRows,
        startY: 34,
    ...
    if (format === 'html') {
      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${_escHtml(filenameBase)}</title>
        ...
        <h2>${_escHtml(filenameBase.replace(/_/g, ' '))}</h2>
        <table><thead><tr>${headers.map(h => `<th>${_escHtml(h)}</th>`).join('')}</tr></thead>
